...
document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    if (typeof initCalendarNavigation === "function") initCalendarNavigation();
  } catch (e) {}
  try {
  ...
  try {
    ...
    if (btn) btn.addEventListener("click", renderReports);
  } catch (e) {}
  try {
  ...
  try {
    const btnGen = document.getElementById("btn-generate-from-analysis");
    if (btnGen)
      btnGen.addEventListener("click", () => openAnalysisGenerateModal());
  } catch (e) {}
  try {
    runAnalysis();
    window.__analysisInterval = setInterval(runAnalysis, 60 * 1000);
  } catch (e) {}

  // global search wiring
  try {
    document.querySelectorAll(".app-search").forEach((inp) => {
